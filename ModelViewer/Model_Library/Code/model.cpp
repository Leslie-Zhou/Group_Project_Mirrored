...
/** @file
 * This contains the code for the model class.
 */
...
void model::exportDataToObj(int id, char type, int mat, int cell_vertices[8])
{
    ...
    cellObjects[id].setCell(id, type, mat, cellVectors, cell_vertices);
}
...
// To retrieve vector information of the model
float model::getX(int id)
{
  return vectorObjects[id].getX();
}

float model::getY(int id)
{
  return vectorObjects[id].getY();
}

float model::getZ(int id)
{
  return vectorObjects[id].getZ();
}

// To retrieve the vertices of the cells
int* model::getCellVertices(int id)
{
  return cellObjects[id].getCellVertices();
}

// To retrieve the cell shape
char model::getCellType(int id)
{
  return cellObjects[id].getShape();
}

// To retrieve the cell colour
string model::getCellColour(int id)
{
  return materialObjects[cellObjects[id].getMat()].GetColour();
}
// --------------
